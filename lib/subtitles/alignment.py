...
from lib.subtitles.domain import TimedWord
        ...
        try:
            if WhisperAlignmentProvider._fw_model is None:
                ...
                try:
                    ...
                    import sys
                    extra_paths = [
                        os.environ.get("WHISPER_EXTRA_PATH"),
                        str(pathlib.Path.home() / ".local" / "lib" / "python3.11" / "site-packages"),
                        "/Users/huutq/Desktop/WorkingSpace/Taka/taka-tales/env/lib/python3.11/site-packages",
                    ]
                    for ep in extra_paths:
                        if ep and ep not in sys.path and pathlib.Path(ep).exists():
