"""
upload_bbc_60s_grizzly_bear.py
Uploads the 60.0s BBC-compliant Grizzly Bear Short to YouTube Shorts.
"""
...
video_file = ROOT / "projects" / "grizzly_bear" / "renders" / "grizzly_bear_bbc_60s_master.mp4"

print(f"🚀 Uploading {video_file.name} ({video_file.stat().st_size / (1024*1024):.1f} MB) to YouTube Shorts...")
...
if res.success:
    ...
    tg_msg = (
        f"🎬 *New BBC-Compliant Short Published (60s)\\!*\n\n"
        f"🐻 *Title:* Why Salmon Willingly Jump Into A Bear's Mouth\n"
        f"⏱️ *Duration:* 60\\.0s \\(57\\.0s BBC footage \\+ 3\\.0s CTA\\)\n"
