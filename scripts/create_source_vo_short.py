...
def render_source_vo_short(
    ...
    # ponytail: single CTA append in render filter, no per-story ASS edits; upgrade to per-topic voice CTA + ElevenLabs if conversion needs lift
    ...
    # Best CTA — proven 0.46% vs 0% on BBC cuts (sJM2/i7X vs 423G)
    if cta_text:
        cta_start = max(0, duration - 3.5)
        cta_esc = cta_text.replace(":", "\\:").replace("'", "\\'").replace("%", "\\%")
        font_arg = ""
        for cand in ["C:/Windows/Fonts/impact.ttf", "C:/Windows/Fonts/arial.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf"]:
            if Path(cand).exists():
                ...
                break
        filter_complex = filter_complex.replace("[v]", f",drawtext=text='{cta_esc}'{font_arg}:fontsize=52:fontcolor=white:borderw=3:bordercolor=black:box=1:boxcolor=black@0.58:boxborderw=18:x=(w-text_w)/2:y=h-280:enable='gte(t\\,{cta_start})'[v]")
