"""
upload_cold_hook_grizzly_bear.py
Uploads the 58.96s Cold Hook Master Grizzly Bear Short to YouTube.
"""
...
video_file = ROOT / "projects" / "grizzly_bear" / "renders" / "grizzly_bear_cold_hook_58s_master.mp4"

print(f"🚀 Uploading Cold Hook Master ({video_file.stat().st_size / (1024*1024):.1f} MB, Duration: 58.9s) to YouTube Shorts...")
...
if res.success:
    ...
    tg_msg = (
        f"🎬 *New Cold Hook Short Published\\!*\n\n"
        f"🐻 *Title:* Why Salmon Willingly Jump Into A Bear's Mouth\n"
        f"⏱️ *Structure:* 3\\.2s Cold Action Hook \\+ 53\\.3s Story \\+ 2\\.3s CTA \\= 58\\.9s Total\n"
