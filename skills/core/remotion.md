...
- **`<Audio>` rejects `file://` URIs — stage audio in `public/`.** The renderer's asset downloader only supports http(s), so absolute-path audio fails mid-render with "Can only download URLs starting with http:// or https://" (misleadingly, `<Img>` tolerates `file://`, so a composition can render stills fine and still die on narration). Copy narration/music/SFX into `remotion-composer/public/<project-slug>/` and reference them staticFile-relative (`"my-slug/narration.mp3"`). Convention: per-project subdir + `public/<brand>-shared/` for reused brand assets (SFX, skyline).

## Vertical (9:16) Format Rules — learned 2026-07 building the CL000 portfolio

These were discovered the hard way (three user-visible font/legibility rounds). All existing components have been retuned; follow these rules when authoring NEW components or compositions for 1080×1920.

1. **SVG chart components render at ~0.52× on vertical.** `BarChart`/`LineChart`/`PieChart` draw on a `viewBox="0 0 1920 1080"` that contain-fits into the frame — at 1080 wide that's a 0.5208 downscale. Font sizes inside those SVGs must be ~2× the desired on-screen pixels. Current tuned values: title 120 (≈62px effective), value labels 96, category labels 80, axis/legend 64. If you add a new SVG scene, apply the same multiplier — or better, derive geometry from `useVideoConfig()` like `KPIGrid` now does.
2. **HTML components render at true frame pixels.** Readability floor on 1080-wide vertical (user-validated): body/label text ≥38px, captions 42px. Constrain long single-line text with `maxWidth` (~86%) so bigger fonts wrap instead of clipping at frame edges.
3. **Give chart titles clearance.** Large titles collide with value labels floating above the tallest bar/line. Tuned: `chartTop = 340` (bar) / `300` (line), pie `cy = 580` when a title is present.
4. **Define `primaryColor` in custom `themeConfig`, or set per-cut colors.** Anything that falls back to `theme.primaryColor` (ComparisonCard `rightColor`, the animated background gradient) silently inherits the default purple when a custom theme omits it. For gold-right comparisons pass `rightColor` on the cut.
5. **Merge WhisperX punctuation tokens before captioning.** WhisperX splits "$301,000" into `$301` + `,000` and "7.4%" into pieces; glue any token starting with `, . %` (consider leading `-` for street numbers) onto the previous word. See `merge_punct_leading` in `projects/video-service-business/portfolio/produce.py`.
5b. **Correct captions against the authored script — Whisper mishears.** Captions transcribed from TTS audio WILL contain mistranscriptions of a script you wrote ("Median list price" → "Media and list price"; "Price it right" → "Price at right"). Never ship raw transcript captions when the script is known: align transcript→script with difflib and replace mismatched words while keeping Whisper's timings AND its number formatting ("29", "$301K" read better on screen than the spelled-out TTS text "twenty-nine"). Implementation: `correct_captions` in `portfolio/produce.py`.
5c. **Per-character text animation must nest chars inside per-word wrappers.** A flat flex of character `<span>`s line-wraps mid-word ("sel / l?") and any "color the first N characters" accent splits words ("Meet Ger" cyan + "man Village" white). Pattern: outer flex (`flexWrap: wrap`, `columnGap: 0.28em`) of word spans (`inline-flex`, `nowrap`), chars animated inside; accent by word index. `HeroTitle.tsx` implements this.
6. **Per-section TTS = exact scene boundaries with zero alignment work.** Generate TTS per script section, probe each duration, boundaries = cumulative times, concat with `apad` gaps, transcribe the concat for captions. Kills the fragile transcript-to-scene alignment step entirely. Reference implementation: `portfolio/produce.py`.
7. **Stock assets:** `pexels_video` — always pass `preferred_quality: "uhd"` and verify returned width ≥1080 (the API's renditions are inconsistently labeled; the tool now falls back to highest-res, but verify). **Integrity rule:** Pexels queries for named places return lookalikes from anywhere on earth ("German Village Columbus" → Quedlinburg, Germany). Never present a lookalike as a specific real place — use verified imagery for place claims, generic imagery for mood.
8. **New template capabilities** (see `remotion-composer/SCENE_TYPES.md`): top-level `brandWatermark` prop (corner brand pill on every scene) and `audio.sfx` one-shot cues (whoosh/pop; generate via `tools/audio/sfx_gen.py`, ElevenLabs, ~$0.02/effect, `prompt_influence: 0.8` for one-shots). Recipe that works: whoosh at chart grow-in (vol 0.4), pop at key stat (0.45), soft whoosh at CTA (0.35) — verify embedding via RMS delta at the cue.
